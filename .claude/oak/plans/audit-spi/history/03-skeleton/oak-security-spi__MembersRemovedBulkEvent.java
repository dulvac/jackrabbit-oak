...
/**
 * Bulk audit event recorded when multiple authorizables are removed
 * from a group in a single API call. Recorded at the API call site
 ...
 * {@code MembershipWriter.removeMembers} could not stage — non-member
 * IDs, invalid references, etc.). It does <em>not</em> include
 * downstream validator rejections: a validator failure rolls back the
 * entire commit, the audit event is dropped via
...
public final class MembersRemovedBulkEvent extends SecurityAuditEvent {
     ...
     */
    public static final String TYPE = "user.members.removed.bulk";
    ...
    private MembersRemovedBulkEvent(@NotNull String groupPath,
     ...
     */
    @NotNull
    public static MembersRemovedBulkEvent of(@NotNull String groupPath,
                                             @NotNull Set<String> memberIds,
                                             boolean isContentId,
                                             @NotNull Set<String> failedIds) {
        ...
        return new MembersRemovedBulkEvent(groupPath, memberIds, isContentId, failedIds);
    }
